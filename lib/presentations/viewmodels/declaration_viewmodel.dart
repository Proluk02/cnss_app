...
class RapportDeclaration {
  ...
  // NOUVEAUX CHAMPS
  final double arrierees;
  final double taxationOffice;
  ...
  RapportDeclaration({
    ...
    // NOUVEAUX CHAMPS
    this.arrierees = 0.0,
    this.taxationOffice = 0.0,
    this.majorationRetard = 0.0,
    this.nombreEnfants = 0,
  });

  double get montantTotalAPayer =>
      totalDesCotisations + arrierees + taxationOffice + majorationRetard;
  ...
  Map<String, dynamic> toMap() {
    return {
      ...
      'arrierees': arrierees,
      'taxationOffice': taxationOffice,
      'majorationRetard': majorationRetard,
  ...
  factory RapportDeclaration.fromMap(Map<String, dynamic> map) {
    return RapportDeclaration(
      ...
      arrierees: (map['arrierees'] ?? 0.0).toDouble(),
      taxationOffice: (map['taxationOffice'] ?? 0.0).toDouble(),
      majorationRetard: (map['majorationRetard'] ?? 0.0).toDouble(),
      nombreEnfants: map['nombreEnfants'] ?? 0,
    );
  }
}
...
class DeclarationViewModel extends ChangeNotifier {
  ...
  String get periodeAffichee => periodeActuelle != null
  ...
  Future<void> initialiser() async {
    ...
    try {
      ...
      if (derniereDeclaration != null &&
        ...
        periodeActuelle = (dernierePeriodeDeclaree == null)
      ...
      tousLesTravailleurs = (await _firebase.getTousLesTravailleurs(
        uid,
      ))
          .map((data) => TravailleurModele.fromMap(data))
          .toList();
  ...
  Future<void> chargerBrouillon({bool notify = true}) async {
    ...
    final brouillonSauvegarde = (await _firebase.getTousLesBrouillons(uid))
    ...
    for (var travailleur in tousLesTravailleurs) {
      final entreeExistante = brouillonSauvegarde.firstWhere(
        ...
        orElse: () => DeclarationTravailleurModele(
  ...
  Future<void> finaliserDeclaration() async {
    ...
    notifyListeners();
    try {
      final lignesValides = brouillonActuel.values
  ...
  RapportDeclaration _calculerRapportFinal(
    ...
    final totalEnfants = tousLesTravailleurs.fold<int>(
      0,
      (sum, t) => sum + t.enfantsBeneficiaires,
    );
    ...
    return RapportDeclaration(
      ...
      totalDesCotisations: totalDesCotisations,
      motifRejet: null,
      nombreEnfants: totalEnfants,
      // Les champs comme arrierees, taxationOffice, etc. sont laissés à 0.0 par défaut
      // car ils seront saisis par les agents de la CNSS, pas par l'employeur.
    );
  }
}
